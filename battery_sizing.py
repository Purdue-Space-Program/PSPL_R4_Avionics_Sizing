...
def main() -> None:
    ...
    batteries = pd.read_excel('inputs.xlsx').drop_duplicates()

    battery_filter(batteries, 400).sort_values(by='Capacity(mAh)', ascending=False).to_excel('battery_options.xlsx')
...
def battery_filter(battery_options: pd.DataFrame, target_voltage: int) -> pd.DataFrame:
    ...
    for _index, cell in battery_options.iterrows():
        num_batteries_required = math.ceil(target_voltage / cell['Voltage(V)'])
        max_current = cell['Max Discharge Current(A)']
        ...
        total_volume_cm = ((cell['Thickness(mm)'] * cell['Width(mm)'] * cell['Length(mm)'] / 1000) * num_batteries_required) + ((math.ceil(num_batteries_required / 2) - 1) * 0.2) 
        ...
        max_time = cell['Capacity(mAh)'] / 1000 * 60 * 60 / target_current
